...
#define READ_EOSTABLE_MODE_HDF5( NAME,OFF )                                          \
   ...
   if ( !(alltables_mode_temp = (double*)malloc(nrho * nmode * nye * 3 * sizeof(double))) )
   {
      fprintf(stderr, "Cannot allocate memory for EOS table\n");
      abort();
   }
   ...
   hsize_t table_dims_mode[2] = {3, nrho * nmode * nye};
   hsize_t var3_mode[2]       = {1, nrho * nmode * nye};
   ...
   if ( !(alltables_mode = (double*)malloc(nrho * nmode * nye * 3 * sizeof(double))) )
   {
      fprintf(stderr, "Cannot allocate memory for EOS table\n");
      abort();
   }
   ...
   for(int i = 0; i < nrho ; i++)
   {
      int indold = i + nrho*(j + nmode*(k + nye*iv));
      int indnew = iv + 3*(i + nrho*(j + nmode*k));
