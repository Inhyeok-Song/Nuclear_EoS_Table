...
{
   // helper variables
   double fh[8][16], delx, dely, delz, a[8];
...
// determine spacing parameters of equidistant (!!!) table
   ...
   idx[0] = NUC_TABLE_NPTR*(ix + nx*(iy + ny*iz));
   idx[1] = NUC_TABLE_NPTR*((ix-1) + nx*(iy + ny*iz));
   idx[2] = NUC_TABLE_NPTR*(ix + nx*((iy-1) + ny*iz));
   idx[3] = NUC_TABLE_NPTR*(ix + nx*(iy + ny*(iz-1)));
   idx[4] = NUC_TABLE_NPTR*((ix-1) + nx*((iy-1) + ny*iz));
   idx[5] = NUC_TABLE_NPTR*((ix-1) + nx*(iy + ny*(iz-1)));
   idx[6] = NUC_TABLE_NPTR*(ix + nx*((iy-1) + ny*(iz-1)));
   idx[7] = NUC_TABLE_NPTR*((ix-1) + nx*((iy-1) + ny*(iz-1)));
