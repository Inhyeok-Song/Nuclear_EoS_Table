...
void write_eos_table( char *nuceos_eps_table_name ) 
{
   ...
   hsize_t   dims_mode[3] = {nye2, nmode, nrho2};
   ...
   if (!(eos_table_mode = (double****)malloc(nye2 * sizeof(double***))))
   {
      fprintf(stderr, "Cannot allocate memory for EOS table\n");
      abort();
   }
   for (int i=0; i<nye2; i++)
   {
      ...
      {
         if (!(eos_table_mode[i][j] = (double**)malloc(nrho2 * sizeof(double*))))
         {
            fprintf(stderr, "Cannot allocate memory for EOS table\n");
            abort();
         }
   ...
   double *wdata_mode;
   if (!(wdata_mode = (double*)malloc(nye2 * nmode * nrho2 * sizeof(double))))
   {
      fprintf(stderr, "Cannot allocate memory for EOS table\n");
      abort();
   }
...
#define WRITE_EOSTABLE_MODE_HDF5(NAME, OFF)                                                           \
   ...
   {                                                                                                  \
      for(int i = 0; i < nye2; i++)                                                                   \
         for(int j = 0; j < nmode; j++)                                                               \
            for(int k = 0; k < nrho2; k++)                                                            \
               wdata_mode[i*nmode*nrho2 + j*nrho2 + k] = eos_table_mode[i][j][k][OFF];                \
